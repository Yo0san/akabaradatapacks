...
execute anchored eyes positioned ^ ^ ^-1 run summon minecraft:marker ~ ~ ~ {Tags:[tf,rocket]}
scoreboard players operation @e[type=minecraft:marker,tag=tf] UID = @s UID
scoreboard players set @e[type=minecraft:marker,tag=tf] cooldown 12
...
scoreboard players operation v11 V = rocket_ticks S

execute as @e[type=minecraft:marker,tag=tf] run function mgs:core/fire_helper

tag @e[type=minecraft:marker,tag=rocket,tag=fired] remove fired