...
data modify storage mgs:tmp arr append from entity @s Inventory[{id:"minecraft:clock",tag:{CustomModelData:130}}].tag.Damage
clear @s clock{CustomModelData:130} 1
scoreboard players operation v0 V = @e[type=minecraft:marker,tag=curr,limit=1] mag
execute if score v0 V >= enableEmptyMagazines V run function mgs:guns/013/gen/mag
